...
const registrarEntrada = async (req, res) => {
    try {
        ...
        const { produto, movimento } = await registrarMovimento({
            produtoId, tipo: 'entrada', quantidade, motivo, observacao, usuarioId: req.user?.id,
        });

        return res.status(201).json({ success: true, produto, movimento, message: 'Entrada registada com sucesso' });
    } catch (error) {
