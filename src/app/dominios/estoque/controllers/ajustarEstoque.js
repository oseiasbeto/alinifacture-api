...
const ajustarEstoque = async (req, res) => {
    try {
        ...
        const { produto, movimento } = await registrarMovimento({
            produtoId, tipo: 'ajuste', quantidade, motivo: motivo || 'Ajuste manual', observacao, usuarioId: req.user?.id,
        });

        return res.status(201).json({ success: true, produto, movimento, message: 'Estoque ajustado com sucesso' });
    } catch (error) {
        console.error('Erro ao registrar ajuste de estoque:', error);
        return res.status(500).json({ success: false, message: 'Erro interno ao registrar ajuste de estoque' });
    }
};
